...
static memoryBlockHeader* heaps[3];
static memoryBlockHeader* freeLists[3];
    ...
    for (int i = 0; i < managedListSize; i++) {
        if (managedList[i] != NULL) {
            ...
            if (oldBlock->survivalAmt >= 3) {
                void* oldPtr = duMalloc(oldBlock->size, 2);
                if (!oldPtr) {
                    fprintf(stderr, "Failed to allocate in old heap\n");
                    return;
                }

                memoryBlockHeader* newBlock = (memoryBlockHeader*)((char*)oldPtr - sizeof(memoryBlockHeader));
...
void majorCollection() {
    memoryBlockHeader* current = heaps[2];
    ...
    while (current) {
        if (current->free) {
            ...
            if (firstFree) {
                memcpy(firstFree, current, sizeof(memoryBlockHeader) + current->size);
                managedList[firstFree->managedIndex] = (void*)((char*)firstFree + sizeof(memoryBlockHeader));
                firstFree->next = current->next;
                firstFree = (memoryBlockHeader*)((char*)firstFree + sizeof(memoryBlockHeader) + firstFree->size);
            }
        }
        current = current->next;
    }

    // Create a free block at the end of the compacted heap
    if (firstFree) {
        firstFree->free = 1;
        firstFree->size = heaps[2]->size - ((char*)firstFree - (char*)heaps[2]) - sizeof(memoryBlockHeader);
        firstFree->managedIndex = -1;
        firstFree->survivalAmt = 0;
        firstFree->next = NULL;
        freeLists[2] = firstFree;
    }
}
